            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#f0f0f0] flex flex-col gap-4 flex-1 min-h-0">
                ...
                {materialsData && (
                    <div className="flex items-center justify-between mt-auto pt-6 shrink-0 border-t border-[#f0f0f0]">
                        <Button variant="primary" disabled={materialsPage === 1} onClick={() => setMaterialsPage(p => Math.max(1, p - 1))} className="px-4 py-2">
                            <MdChevronLeft size={24} className="-ml-1" />
                            <span className="hidden sm:inline">Previous</span>
                        </Button>
                        <span className="text-base font-bold text-[#0F1D29] font-body bg-[#FFF7E6] border border-[#e8d5b5] px-5 py-2 rounded-xl shadow-sm">Page {materialsData.currentPage} of {materialsData.totalPages}</span>
                        <Button variant="primary" disabled={materialsPage === materialsData.totalPages} onClick={() => setMaterialsPage(p => Math.min(materialsData.totalPages, p + 1))} className="px-4 py-2">
